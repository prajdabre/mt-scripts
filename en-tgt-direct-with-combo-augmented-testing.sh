...
for i in phrase_based/*-en; do
        ...
        mkdir $i/comboallwithdirect;
        cd $i/comboallwithdirect;
	echo "Doing mert on pair $i";
        ~/tools/moses/scripts/training/mert-moses.pl ~/phrase-tables-LMs/corpora/$pair/tun.$src ~/phrase-tables-LMs/corpora/$pair/tun.$tgt ~/tools/moses/bin/moses ../moses-combo-augments-direct.ini --mertdir ~/tools/moses/bin/ --rootdir ~/tools/moses/scripts/ -no-filter-phrase-table --batch-mira --return-best-dev --batch-mira-args '-J 100' --decoder-flags '-threads 4 -v 0';
	echo "Testing pair $i";
        ...
        echo "When all is combined for pair along with direct system on full corpus: $pair" >> ../combo-with-direct-full-bleu-score;
        cat bleu-score >> ../combo-with-direct-full-bleu-score;
